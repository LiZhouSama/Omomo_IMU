...
class TransformerDiffusionModel(nn.Module):
    def __init__(
        self,
        d_input_feats,  # 输入特征维度：全身姿态维度 × 2 (条件部分 + 预测部分)
        d_feats,        # 输出维度：24个关节位置(24×3) + 22个关节旋转(22×6)
        ...
        self.d_feats = d_feats  # 24*3 + 22*6 = 204
...
class CondGaussianDiffusion(nn.Module):
    def __init__(
        ...
        d_input_feats = 2*d_feats  # 输入维度是输出维度的两倍，包含条件和预测部分
        
        self.denoise_fn = TransformerDiffusionModel(
            d_input_feats=d_input_feats,  # 408
            d_feats=d_feats,              # 204
            d_model=d_model,
            n_head=n_head,
            d_k=d_k,

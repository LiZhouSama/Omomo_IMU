...
def compute_s1_metrics(pred_joints, gt_joints):
    """
    计算IMU关节(双手腕和颈部)的位置误差
    
    参数:
        pred_joints: T X N X 3 (预测的左手腕、右手腕、颈部位置)
        gt_joints: T X N X 3 (真实的左手腕、右手腕、颈部位置)
        
    返回:
        imu_jpe: 所有IMU关节的平均位置误差
    """
    # 计算每个关节的欧氏距离误差
    joint_errors = torch.norm(pred_joints - gt_joints, dim=-1)  # T X 3
    
    # 计算所有IMU关节的平均误差
    imu_jpe = joint_errors.mean().item()
